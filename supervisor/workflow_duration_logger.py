...
class WorkflowDurationMetrics:
    ...
    def complete(self, end_time: Optional[float] = None, success: bool = True, error_message: Optional[str] = None):
...
class WorkflowDurationLogger:
    ...
    def __init__(self, log_file_path: str = "logs/workflow_durations.jsonl", 
        ...
        self.logger.info(f"WorkflowDurationLogger initialized with log file: {self.log_file_path}")
    
    def start_workflow_tracking(self, workflow_id: str, source: WorkflowSource, 
                              workflow_type: WorkflowType = WorkflowType.UNKNOWN,
                              instruction: Optional[str] = None,
                              user_id: Optional[str] = None,
                              channel_id: Optional[str] = None) -> WorkflowDurationMetrics:
        """
        ...
        if self.enable_console_logging:
            self.logger.info(f"Started tracking workflow '{workflow_id}' from {source.value}")
        
        return metrics
    
    def complete_workflow_tracking(self, workflow_id: str, success: bool = True, 
                                 error_message: Optional[str] = None,
                                 role: Optional[str] = None,
                                 confidence: Optional[float] = None,
                                 task_count: Optional[int] = None) -> Optional[WorkflowDurationMetrics]:
        """
        ...
        if workflow_id not in self.active_workflows:
            self.logger.warning(f"Attempted to complete tracking for unknown workflow '{workflow_id}'")
            return None
        ...
        if self.enable_console_logging:
            ...
            self.logger.info(f"Workflow '{workflow_id}' {status} in {metrics.duration_seconds:.2f}s "
    ...
    def get_performance_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        recent_metrics = self.get_recent_metrics(limit=1000)  # Get more data for analysis
        ...
        durations = [m.get('duration_seconds', 0) for m in filtered_metrics if m.get('duration_seconds')]
...
def initialize_duration_logger(log_file_path: str = "logs/workflow_durations.jsonl",
                             enable_console_logging: bool = True,
                             max_log_file_size_mb: int = 100) -> WorkflowDurationLogger:
