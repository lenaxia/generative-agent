...
class Heartbeat:
        ...
        try:
            if hasattr(self.supervisor, 'workflow_engine'):
                ...
                for workflow_id, context in workflow_engine.active_workflows.items():
                    if (hasattr(context, 'completed_at') and
                        context.completed_at and
                        context.completed_at < cutoff_time):
    ...
    def _check_universal_agent_health(self) -> Dict[str, Any]:
        ...
        try:
            if hasattr(self.supervisor, 'workflow_engine'):
                ...
                return {
                    'status': 'healthy' if ua_status.get('universal_agent_enabled') else 'disabled',
    ...
    def _check_mcp_health(self) -> Dict[str, Any]:
        ...
        try:
            if hasattr(self.supervisor, 'workflow_engine'):
                ...
                if hasattr(workflow_engine, 'universal_agent') and hasattr(workflow_engine.universal_agent, 'mcp_client'):
                    mcp_client = workflow_engine.universal_agent.mcp_client
                    
                    if mcp_client:
                        server_status = mcp_client.get_all_server_status()
                        healthy_servers = sum(1 for status in server_status.values() if status.get('status') == 'connected')
                        ...
                        return {'status': 'disabled', 'message': 'MCP client not initialized'}
                else:
                    return {'status': 'disabled', 'message': 'MCP integration not available'}
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'uptime': time.time() - getattr(self, 'start_time', time.time()),
            'last_heartbeat': self.metrics.get('heartbeat', {}).get('last_heartbeat', 0)
        }
