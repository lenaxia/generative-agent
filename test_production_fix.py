...
async def test():
    ...
    try:
        ...
        if hasattr(universal_agent, '_current_domain_role') and universal_agent._current_domain_role:
            ...
            print(f"   Domain role type: {type(universal_agent._current_domain_role).__name__}")
            print(f"   Tools available: {len(universal_agent._current_domain_role.tools)}")
            print(f"   Required tools: {universal_agent._current_domain_role.REQUIRED_TOOLS}")
            return 0
        else:
