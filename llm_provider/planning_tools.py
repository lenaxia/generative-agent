...
def create_task_plan(
    ...
    if not available_roles:
        ...
        tasks, dependencies = _create_llm_task_plan(
            instruction, available_roles, llm_factory
        )
    ...
    logger.info(
        f"Created dynamic task plan with {len(tasks)} tasks and {len(dependencies)} dependencies"
    )
    ...
    try:
        ...
        if llm_factory:
            ...
            response = universal_agent.execute_task(
                instruction=planning_prompt, role="planning", llm_type=LLMType.STRONG
            )
            ...
            for dep in plan_data.get("dependencies", []):
                ...
                valid_dep = {"source": dep.get("source"), "target": dep.get("target")}
                if "condition" in dep:
...
def validate_task_plan(
    ...
    logger.info(
        f"Task plan validation: {'PASSED' if is_valid else 'FAILED'} with {len(errors)} errors"
    )
    return result
...
def optimize_task_plan(
    ...
    logger.info(
        f"Task plan optimized: {len(dependencies)} -> {len(optimized_dependencies)} dependencies"
    )
    return result
