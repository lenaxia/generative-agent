"""Request Router Module with improved LLM-based routing.
...
This version includes improvements to ensure all roles are correctly processed.
"""
...
try:
    ...
    Agent = getattr(LLMFactory, "Agent", None)
...
class RequestRouter:
    ...
    Uses STRONG model via LLMFactory for accurate, reliable routing decisions.
    ...
    def route_request(self, instruction: str) -> dict[str, Any]:
        ...
        try:
            ...
            # Use STRONG model type for more reliable role processing
            result = self.universal_agent.execute_task(
                instruction=routing_prompt, role="router", llm_type=LLMType.STRONG
            )
    ...
    def get_routing_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "llm_type_used": LLMType.STRONG.value,
        }
    ...
    def validate_routing_setup(self) -> dict[str, Any]:
        ...
        # Check LLM factory
        try:
            model = self.llm_factory.create_strands_model(LLMType.STRONG)
            if model is None:
                issues.append("STRONG model not available from LLMFactory")
        except Exception as e:
            issues.append(f"Failed to create STRONG model: {e}")
    ...
    def _build_enhanced_routing_prompt(
        ...
        Improved prompt structure to ensure all roles are properly processed.
        """
        ...
        # Improved prompt structure with clearer instructions and examples
...
IMPORTANT: Consider ALL available roles listed above when making your routing decision.
For a weather-related request like "weather in Seattle", route to the weather role.
For a timer-related request like "set a timer for 5 minutes", route to the timer role.
For a smart home request like "turn on the lights", route to the smart_home role.
For a calendar-related request like "schedule a meeting", route to the calendar role.
...
- Ensure all required parameters are extracted if possible"""
    ...
    def _parse_routing_and_parameters(self, llm_result: str) -> dict[str, Any]:
        ...
        try:
            ...
            # Better handling of JSON extraction from LLM response
