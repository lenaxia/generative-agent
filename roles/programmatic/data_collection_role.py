...
class DataSourceManager:
    ...
    def collect_from_source(self, source: str, data_types: List[str]) -> List[Dict[str, Any]]:
        """
...
class DataCollectionRole(ProgrammaticRole):
    ...
    def _collect_data_parallel(self, params: Dict[str, Any]) -> Dict[str, List[Dict[str, Any]]]:
        """
        ...
        if params.get("parallel", True) and len(params["sources"]) > 1:
            ...
            with ThreadPoolExecutor(max_workers=min(len(params["sources"]), 5)) as executor:
                future_to_source = {
                    executor.submit(self._collect_from_single_source, source, params["data_types"]): source
            ...
            for source in params["sources"]:
                try:
                    data = self._collect_from_single_source(source, params["data_types"])
                    collected_data[source] = data
                except Exception as e:
                    logger.warning(f"Failed to collect from source '{source}': {e}")
    ...
    def _collect_from_single_source(self, source: str, data_types: List[str]) -> List[Dict[str, Any]]:
        """
