...
def summarize_text(text: str, max_length: int = 200, summary_type: str = "general") -> Dict[str, Any]:
    ...
    """
    logger.info(f"Summarizing text: {len(text)} characters, max_length: {max_length}, type: {summary_type}")
    ...
    logger.info(f"Text summarized: {len(words)} -> {len(summary_words)} words ({compression_ratio:.2%} compression)")
    return result
...
def create_bullet_point_summary(text: str, max_length: int) -> str:
    ...
    for sentence in sentences[:5]:  # Max 5 bullet points
        ...
        if current_words + words_in_sentence + 2 <= max_length:  # +2 for bullet formatting
...
def analyze_text_complexity(text: str) -> Dict[str, Any]:
    ...
    logger.info(f"Text complexity analysis: {complexity} ({word_count} words, {sentence_count} sentences)")
    return result
...
def extract_key_phrases(text: str, max_phrases: int = 10) -> Dict[str, Any]:
    ...
    top_phrases = sorted(phrase_counts.items(), key=lambda x: x[1], reverse=True)[:max_phrases]
    ...
    logger.info(f"Key phrases extracted: {len(key_phrases)} phrases from {len(phrase_counts)} total")
    return result
