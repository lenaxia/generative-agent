...
class IntelligentSlackBot:
        ...
        try:
            ...
            if response["ok"]:
                user_info = {
                    ...
                    "real_name": response["user"].get("real_name", response["user"]["name"]),
                    "display_name": response["user"]["profile"].get("display_name", response["user"]["name"])
                }
    ...
    async def _process_with_ai(self, message_text, user_id=None, channel=None, client=None):
        """
        ...
        try:
            ...
            if client:
                resolved_message = await self._resolve_slack_references(message_text, client)
            ...
            while wait_time < max_wait:
                ...
                if task_statuses:
                    completed_tasks = [task_id for task_id, task_status in task_statuses.items()
    ...
    async def _get_workflow_result(self, workflow_id: str) -> str:
        ...
        try:
            ...
            task_context = self.supervisor.workflow_engine.get_request_context(workflow_id)
            ...
            if hasattr(task_context.task_graph, 'progressive_summary') and task_context.task_graph.progressive_summary:
            ...
            try:
                ...
                if status:
                    ...
                    if 'workflow_engine' in status:
                        ...
                        status_msg += f"• Queue Size: {we_status.get('queued_tasks', 0)}\n"
                    
                    if 'universal_agent' in status:
                        ua_status = status['universal_agent']
                        status_msg += f"• Framework: {ua_status.get('framework', 'unknown')}\n"
    ...
    def setup_commands(self):
        ...
        def ai_command(ack, body, say, respond):
            ...
            initial_response = respond(f"<@{user_id}> 🤔 Processing your request...")
            ...
            def process_ai():
                try:
                    ...
                    respond(f"<@{user_id}> Sorry, I encountered an error processing your request.")
