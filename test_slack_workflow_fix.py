...
async def test_slack_message_to_workflow():
    ...
    try:
        if mock_workflow_engine.received_requests:
            ...
            if isinstance(request, RequestMetadata):
                logger.info(
                    "✅ SUCCESS: Slack message properly converted to RequestMetadata"
                )
                logger.info(f"   Prompt: '{request.prompt}'")
                ...
                logger.error(
                    f"❌ FAIL: Received {type(request)} instead of RequestMetadata"
                )
                return False
        else:
...
async def main():
    ...
    try:
        ...
        if success:
            logger.info(
                "🎉 All tests passed! Slack messages should now trigger workflows."
            )
        else:
