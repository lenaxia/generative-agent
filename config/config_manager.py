...
class ConfigManager:
    ...
    def load_config(self, validate: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ConfigValidationError(f"Invalid YAML in config file: {e}", self.config_path)
    ...
    def _validate_config(self):
        ...
        if framework_type not in ["strands", "langchain"]:
            errors.append("framework.type must be 'strands' or 'langchain'")
        ...
        for role, llm_type in role_mapping.items():
            if llm_type not in valid_llm_types:
                errors.append(f"Invalid LLM type '{llm_type}' for role '{role}'. Must be one of: {valid_llm_types}")
        
        # Validate task management settings
        max_concurrent = self.get_config("task_management.max_concurrent_tasks", 5)
        if not isinstance(max_concurrent, int) or max_concurrent <= 0:
            errors.append("task_management.max_concurrent_tasks must be a positive integer")
    ...
    def get_role_llm_mapping(self) -> Dict[str, str]:
        ...
        return self.get_config("universal_agent.role_llm_mapping", {
            "planning": "STRONG",
            "analysis": "STRONG", 
            "coding": "STRONG",
            "search": "WEAK",
