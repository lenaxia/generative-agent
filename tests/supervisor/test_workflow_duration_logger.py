...
class TestWorkflowDurationLogger:
    ...
    def test_multiple_workflows(self):
        ...
        self.logger.complete_workflow_tracking(workflow_ids[2], success=False, error_message="Test error")
        assert self.logger.get_active_workflow_count() == 0
    ...
    def test_get_performance_summary(self):
        ...
        for wf_id in failed_workflows:
            ...
            time.sleep(0.01)
            self.logger.complete_workflow_tracking(wf_id, success=False, error_message="Test error")
    ...
    def test_log_file_rotation(self):
        ...
        for i in range(10):
            ...
            small_logger.start_workflow_tracking(
                workflow_id=wf_id,
                source=WorkflowSource.CLI,
                instruction=f"Long instruction for workflow {i} " * 100  # Make it large
            )
...
class TestWorkflowDurationIntegration:
    ...
    def test_slack_workflow_simulation(self):
        ...
        workflow_type = WorkflowType.FAST_REPLY if workflow_id.startswith('fr_') else WorkflowType.COMPLEX_WORKFLOW
        self.logger.update_workflow_type(workflow_id, workflow_type)
