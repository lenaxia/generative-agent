...
from common.task_graph import TaskDependency, TaskDescription, TaskStatus
...
class TestComprehensiveTaskContext:
    ...
    def test_error_handling_and_recovery(self, task_context):
        ...
        # Verify task failure
        task_context.task_graph.get_node_by_task_id(first_task.task_id)
    ...
    def test_integration_with_strands_agent(self, task_context):
        ...
        ready_tasks = task_context.get_ready_tasks()
        if ready_tasks:
            context_for_agent = task_context.prepare_task_execution(
                ready_tasks[0].task_id
            )
        else:
