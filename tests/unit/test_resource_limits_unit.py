...
class TestResourceLimitsUnit:
    ...
    def large_task_context(self):
        ...
        for i in range(20):
            task = TaskDescription(
                ...
                prompt=f"This is task {i} for resource limit testing. "
                * 10,  # Make prompt longer
            )
            tasks.append(task)
            ...
            if i > 0:
                dependency = TaskDependency(
                    source=f"Resource Test Task {i-1}", target=f"Resource Test Task {i}"
                )
                dependencies.append(dependency)

        return TaskContext.from_tasks(
            tasks=tasks, dependencies=dependencies, request_id="resource_test_request"
        )
    ...
    def test_task_timeout_handling(self, workflow_engine):
        ...
        context = TaskContext.from_tasks(
            tasks=[task], dependencies=[], request_id="timeout_test"
        )
        ...
        workflow_engine.universal_agent.execute_task.side_effect = TimeoutError(
    ...
    def test_checkpoint_size_limits(self, large_task_context):
        ...
        for i in range(500):
            large_task_context.add_user_message(
                f"Large message {i} with substantial content: {'data' * 100}"
            )
    ...
    def test_concurrent_access_safety(self, mock_start_workflow, workflow_engine):
        ...
        mock_start_workflow.side_effect = (
            lambda prompt: f"wf_mock_{hash(prompt) % 10000:04d}"
        )
    ...
    def test_memory_cleanup_on_completion(self, workflow_engine):
        ...
        for i in range(10):
            ...
            mock_context.get_performance_metrics.return_value = {
        ...
        try:
            workflow_engine.cleanup_completed_requests(
                max_age_seconds=0
            )  # Cleanup immediately
