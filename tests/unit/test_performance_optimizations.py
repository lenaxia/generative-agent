...
class TestLLMFactoryPerformance:
    ...
    def test_model_caching_performance(self):
        ...
        assert second_call_time < first_call_time / 2, f"Cached call ({second_call_time:.4f}s) should be much faster than first call ({first_call_time:.4f}s)"
        
        # Verify cache statistics
        stats = self.factory.get_cache_stats()
        assert stats["models_cached"] >= 1, "Should have at least one cached model"
        assert "weak_default" in stats["model_cache_keys"], "Should have weak model cached"
    ...
    def test_model_warming(self):
        ...
        start_time = time.time()
        model = self.factory.create_strands_model(LLMType.WEAK)
        ...
        if warming_time > 0.001:  # Only check if warming took measurable time
            assert cached_call_time < warming_time / 2, f"Cached call ({cached_call_time:.4f}s) should be faster than warming ({warming_time:.4f}s)"
        else:
            # Both are very fast, just verify cache hit
            assert cached_call_time < 0.01, f"Cached call should be very fast: {cached_call_time:.4f}s"
...
class TestRoleRegistryPerformance:
    ...
    def setup_method(self):
        """Set up test fixtures."""
        with patch('llm_provider.role_registry.Path.exists', return_value=True), \
    ...
    def test_initialize_once_performance(self):
        ...
        with patch.object(self.registry, 'refresh', side_effect=mock_refresh) as mock_refresh_obj:
            # First call should trigger refresh
            self.registry.initialize_once()
            assert mock_refresh_obj.call_count == 1, "First initialize_once should call refresh"
            
            # Subsequent calls should not trigger refresh
            self.registry.initialize_once()
            self.registry.initialize_once()
            assert mock_refresh_obj.call_count == 1, "Subsequent initialize_once calls should not call refresh"
    ...
    def test_fast_reply_roles_caching(self):
        ...
        assert second_call_time < first_call_time or second_call_time < 0.001, "Cached call should be faster or negligible"
...
class TestWorkflowEnginePerformance:
    ...
    def test_initialization_performance(self, mock_mcp, mock_router, mock_agent):
        ...
        with patch('supervisor.workflow_engine.RoleRegistry', return_value=mock_registry):
            ...
            assert init_time < 2.0, f"Initialization took {init_time:.2f}s, should be under 2s with optimizations"
...
class TestEndToEndPerformance:
    ...
    @patch('llm_provider.role_registry.Path.iterdir', return_value=[])
    @patch('llm_provider.role_registry.Path.glob', return_value=[])
    def test_routing_performance_target(self, mock_glob, mock_iterdir, mock_exists, mock_mcp, mock_router, mock_agent):
        ...
        assert routing_time < 1.0, f"Routing took {routing_time:.3f}s, should be under 1.0s (target: <1s, original: ~5s)"
