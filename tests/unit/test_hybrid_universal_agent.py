...
class TestHybridUniversalAgent:
    ...
    def mock_role_registry(self):
        ...
        registry.get_role.side_effect = lambda name: llm_role_def if name == "test_llm_role" else None
        registry.get_programmatic_role.side_effect = lambda name: prog_role if name == "test_prog_role" else None
        registry.is_programmatic_role.side_effect = lambda name: name == "test_prog_role"
        registry.get_role_type.side_effect = lambda name: "programmatic" if name == "test_prog_role" else "llm"
    ...
    def test_execute_llm_task(self, universal_agent, mock_llm_factory):
        ...
        with patch.object(universal_agent, 'assume_role', return_value=mock_agent) as mock_assume:
    ...
    def test_execute_task_hybrid_routing(self, universal_agent):
        ...
        with patch.object(universal_agent, 'execute_programmatic_task', return_value="prog_result") as mock_prog:
            ...
            mock_prog.assert_called_once_with("test instruction", "test_prog_role", mock_context)
        
        # Test LLM routing
        with patch.object(universal_agent, 'execute_llm_task', return_value="llm_result") as mock_llm:
            result = universal_agent.execute_task(
                instruction="test instruction",
                role="test_llm_role",
                llm_type=LLMType.DEFAULT,
            ...
            mock_llm.assert_called_once_with("test instruction", "test_llm_role", LLMType.DEFAULT, mock_context)
    ...
    def test_register_programmatic_role(self, universal_agent):
        ...
        universal_agent.role_registry.register_programmatic_role.assert_called_once_with(mock_role)
    ...
    def test_hybrid_execution_performance_tracking(self, universal_agent):
        ...
        with patch.object(universal_agent, 'execute_programmatic_task', return_value="prog_result"):
            universal_agent.execute_task("test", "test_prog_role", LLMType.DEFAULT, mock_context)
        
        # Execute LLM task
        with patch.object(universal_agent, 'execute_llm_task', return_value="llm_result"):
            universal_agent.execute_task("test", "test_llm_role", LLMType.DEFAULT, mock_context)
    ...
    def test_error_handling_programmatic_execution_failure(self, universal_agent):
        ...
        universal_agent.role_registry.get_programmatic_role.side_effect = lambda name: failing_role if name == "failing_role" else None
    ...
    def test_fallback_to_llm_on_programmatic_failure(self, universal_agent):
        ...
        with patch.object(universal_agent, 'execute_programmatic_task', side_effect=Exception("Prog failed")):
            with patch.object(universal_agent, 'execute_llm_task', return_value="llm_fallback"):
                # This test verifies the concept - actual fallback implementation would be in execute_task
                try:
                    universal_agent.execute_programmatic_task("test", "test_prog_role", None)
                    assert False, "Should have raised exception"
                except Exception as e:
                    assert "Prog failed" in str(e)
                    
                    # Fallback would work
                    fallback_result = universal_agent.execute_llm_task("test", "test_llm_role", LLMType.DEFAULT, None)
