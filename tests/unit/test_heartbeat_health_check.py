...
class TestHeartbeatHealthCheck:
    ...
    def setup_method(self):
        ...
        self.heartbeat = Heartbeat(self.mock_supervisor, interval=30, health_check_interval=60)
    ...
    def test_healthy_components_return_healthy_status(self):
        ...
        with patch.object(self.heartbeat, '_check_supervisor_health') as mock_supervisor, \
             patch.object(self.heartbeat, '_check_workflow_engine_health') as mock_workflow, \
    ...
    def test_disabled_mcp_does_not_cause_degraded_status(self):
        ...
        with patch.object(self.heartbeat, '_check_supervisor_health') as mock_supervisor, \
            ...
            mock_workflow.return_value = {'status': 'healthy'}
            mock_agent.return_value = {'status': 'healthy'}
            mock_mcp.return_value = {'status': 'disabled', 'message': 'MCP integration not available'}
    ...
    def test_unhealthy_component_causes_degraded_status(self):
        ...
        with patch.object(self.heartbeat, '_check_supervisor_health') as mock_supervisor, \
            ...
            mock_supervisor.return_value = {'status': 'healthy'}
            mock_workflow.return_value = {'status': 'unhealthy', 'error': 'Connection failed'}
            mock_agent.return_value = {'status': 'healthy'}
    ...
    def test_health_check_metrics_stored_correctly(self):
        ...
        with patch.object(self.heartbeat, '_check_supervisor_health') as mock_supervisor, \
            ...
            mcp_result = {'status': 'disabled', 'message': 'MCP integration not available'}
            ...
            assert health_check_metrics['components']['workflow_engine'] == workflow_result
