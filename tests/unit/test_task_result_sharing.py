...
class TestTaskResultSharing:
    ...
    def test_dependent_task_receives_predecessor_results(
        self, workflow_engine, mock_universal_agent
    ):
        ...
        task_context = TaskContext.from_tasks(
            tasks, dependencies, request_id="test_request"
        )
    ...
    def test_first_task_receives_no_predecessor_results(
        self, workflow_engine, mock_universal_agent
    ):
        ...
        task_context = TaskContext.from_tasks(
            tasks, dependencies=[], request_id="independent_test"
        )
    ...
    def test_multiple_predecessor_results_included(
        self, workflow_engine, mock_universal_agent
    ):
        ...
        dependencies = [
            TaskDependency(
                source="Search for location info", target="Analyze combined data"
            ),
            TaskDependency(source="Get weather data", target="Analyze combined data"),
        ]

        task_context = TaskContext.from_tasks(
            tasks, dependencies, request_id="multi_test"
        )
    ...
    def test_empty_predecessor_results_handling(
        self, workflow_engine, mock_universal_agent
    ):
        ...
        task_context = TaskContext.from_tasks(
            tasks, dependencies, request_id="empty_test"
        )
...
class TestTaskResultSharingIntegration:
    ...
    def test_real_task_graph_result_sharing(self):
        ...
        task_graph = TaskGraph(
            tasks=tasks, dependencies=dependencies, request_id="integration_test"
        )
        ...
        collection_result = (
            "Data collected: USS Monitor was a revolutionary ironclad warship"
        )
    ...
    def test_task_history_with_include_full_history_flag(self):
        ...
        task_graph = TaskGraph(
            tasks=tasks, dependencies=dependencies, request_id="history_test"
        )
    ...
    def test_performance_optimization_verification(
        self, workflow_engine, mock_universal_agent
    ):
        ...
        dependencies = [
            TaskDependency(
                source="Search for information", target="Analyze information"
            )
        ]

        task_context = TaskContext.from_tasks(
            tasks, dependencies, request_id="performance_test"
        )
...
- Significance: Changed naval warfare forever"""
        ...
        mock_universal_agent.execute_task.return_value = (
            "Analysis based on provided search results"
        )
...
class TestTaskGraphHistoryMechanism:
    ...
    def test_task_graph_history_storage(self):
        ...
        task_graph = TaskGraph(
            tasks=tasks, dependencies=[], request_id="history_storage_test"
        )
    ...
    def test_get_task_history_for_dependent_tasks(self):
        ...
        dependencies = [TaskDependency(source="Producer task", target="Consumer task")]

        task_graph = TaskGraph(
            tasks=tasks, dependencies=dependencies, request_id="dependency_test"
        )
    ...
    def test_task_result_sharing_with_checkpointing(self):
        ...
        dependencies = [TaskDependency(source="Data producer", target="Data consumer")]

        # Create TaskContext
        task_context = TaskContext.from_tasks(
            tasks, dependencies, request_id="checkpoint_test"
        )
        ...
        history = restored_context.task_graph.get_task_history(
            restored_consumer_task.task_id
        )
