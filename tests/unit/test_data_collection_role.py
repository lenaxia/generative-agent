...
class TestDataCollectionRole:
    ...
    def test_parse_instruction_extracts_sources_and_types(self, data_collection_role):
        ...
        instruction = "Collect user data from database and API endpoints with JSON format"
    ...
    def test_collect_from_multiple_sources(self, data_collection_role):
        ...
        data_collection_role.data_sources.collect_from_source.side_effect = mock_collect_from_source
    ...
    def test_structured_data_formatting(self, data_collection_role):
        ...
        required_fields = ["collected_data", "sources", "total_records", "execution_metadata"]
    ...
    def test_error_handling_partial_source_failures(self, data_collection_role):
        ...
        data_collection_role.data_sources.collect_from_source.side_effect = mock_collect_with_failures
        ...
        assert "collected_data" in result
        assert result["total_records"] >= 0  # May have some data from successful sources
    ...
    def test_data_type_filtering_and_validation(self, data_collection_role):
        ...
        data_collection_role.data_sources.collect_from_source.side_effect = mock_collect_typed_data
    ...
    def test_batch_processing_capabilities(self, data_collection_role):
        ...
        def mock_batch_collect(source, data_types):
            return [{"batch": i, "data": f"Batch data {i}", "source": source} 
                   for i in range(5)]  # Return 5 items per source
        
        data_collection_role.data_sources.collect_from_source.side_effect = mock_batch_collect
        ...
        assert "execution_metadata" in result
        execution_time = float(result["execution_metadata"].get("execution_time", "0s").replace("s", ""))
