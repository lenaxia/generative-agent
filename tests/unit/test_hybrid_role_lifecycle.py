...
class TestEnhancedRequestRouter:
    ...
    def test_enhanced_route_request(self):
        ...
        self.universal_agent.execute_task = Mock(
            return_value="""
        {
            "route": "weather",
            "confidence": 0.92,
...
class TestHybridUniversalAgent:
    ...
    def setup_method(self):
        ...
        self.agent = UniversalAgent(
            self.llm_factory, self.role_registry, self.mcp_manager
        )
    ...
    async def test_post_processing_execution(self):
        ...
        result = await self.agent._run_post_processors(
            mock_role_def, lifecycle_functions, "Temperature is 72°F", context, {}
        )
    ...
    def test_data_injection_for_llm_context(self):
        ...
        result = self.agent._inject_pre_data(
            mock_role_def, "What's the weather?", pre_data
        )
    ...
    def test_lifecycle_phase_detection(self):
        ...
        role_def_with_pre.config = {"lifecycle": {"pre_processing": {"enabled": True}}}
        ...
        role_def_with_post.config = {
            "lifecycle": {"post_processing": {"enabled": True}}
        }
...
class TestParameterValidation:
    ...
    def test_enum_parameter_validation(self):
        ...
        # Invalid enum value
        assert (
            self._validate_parameter("next_week", parameter_schema["timeframe"])
            is False
        )
    ...
    def test_required_parameter_validation(self):
        ...
        assert (
            self._validate_required_parameters(
                parameters, {"location": parameter_schema["location"]}
            )
            is True
        )
