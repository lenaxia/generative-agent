...
class TestSearchDataCollectorRole:
    ...
    def mock_search_tools(self):
        ...
        tools.search_source.return_value = [
            {"title": "Test Result 1", "url": "http://example1.com", "snippet": "Test content 1"},
            {"title": "Test Result 2", "url": "http://example2.com", "snippet": "Test content 2"}
        ]
        return tools
    ...
    def test_search_data_collector_initialization(self):
        ...
        assert "collects" in role.description.lower() and "data" in role.description.lower()
    ...
    def test_execute_search_pipeline_pure_data_collection(self, search_data_collector_role):
        ...
        search_data_collector_role.search_tools.search_source.assert_has_calls(expected_calls)
    ...
    def test_needs_followup_logic(self, search_data_collector_role):
        ...
        assert search_data_collector_role._needs_followup(insufficient_results, params) is True
        
        # Test case where follow-up is not needed (sufficient results)
        sufficient_results = [
            {"title": "Result 1"}, {"title": "Result 2"}, 
            {"title": "Result 3"}, {"title": "Result 4"}
        ]
        
        assert search_data_collector_role._needs_followup(sufficient_results, params) is False
    ...
    def test_llm_guided_followup_search(self, search_data_collector_role):
        ...
        followup_params = search_data_collector_role._llm_determine_followup(results, params)
    ...
    def test_error_handling_in_execution(self, search_data_collector_role):
        ...
        search_data_collector_role.search_tools.search_source.side_effect = Exception("Search API failed")
        ...
        assert "search_results" in result
        assert "metadata" in result
        assert result["metadata"]["total_results"] == 0  # No results due to all sources failing
        assert result["execution_type"] == "programmatic_data_collection"
    ...
    def test_structured_data_output_format(self, search_data_collector_role):
        ...
        metadata = result["metadata"]
        required_metadata = ["query", "sources_searched", "total_results", "search_timestamp", "llm_calls"]
    ...
    def test_no_analysis_in_output(self, search_data_collector_role):
        ...
        analysis_fields = ["summary", "analysis", "insights", "recommendations", "conclusions"]
    ...
    def test_multiple_source_integration(self, search_data_collector_role):
        ...
        def mock_search_source(query, source, num_results):
            if source == "web":
                ...
                return [{"title": f"Wikipedia result for {query}", "source": "wikipedia"}]
            else:
                return [{"title": f"Other result for {query}", "source": source}]
        
        search_data_collector_role.search_tools.search_source.side_effect = mock_search_source
    ...
    def test_performance_optimization_minimal_llm_calls(self, search_data_collector_role):
        ...
        if not search_data_collector_role._needs_followup(result["search_results"], {"min_results": 1}):
            assert llm_calls == 1