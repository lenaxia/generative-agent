...
class TestRequestRouter:
    ...
    def mock_role_registry(self):
        ...
        weather_role.config = {
            "role": {"description": "Get weather information", "fast_reply": True}
        }

        calendar_role = Mock(spec=RoleDefinition)
        calendar_role.name = "calendar"
        calendar_role.config = {
            "role": {"description": "Manage calendar events", "fast_reply": True}
        }
    ...
    def test_init(self, mock_llm_factory, mock_role_registry):
        ...
        router = RequestRouter(
            mock_llm_factory, mock_role_registry, mock_universal_agent
        )
    ...
    def test_route_request_agent_creation_error(self, request_router):
        ...
        request_router.universal_agent.execute_task.side_effect = Exception(
            "UniversalAgent execution failed"
        )
...
class TestRequestRouterEdgeCases:
    ...
    def request_router(self):
        ...
        weather_role.name = "weather"
        weather_role.config = {
            "role": {"description": "Weather information", "fast_reply": True}
        }
