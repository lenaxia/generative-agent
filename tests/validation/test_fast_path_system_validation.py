...
class TestFastPathSystemValidation:
    ...
    def test_complete_fast_path_system_validation(
        ...
        with patch("supervisor.workflow_engine.MCPClientManager"):
            ...
            for scenario in fallback_scenarios:
                with patch("llm_provider.request_router.Agent") as mock_agent_class:
                    ...
                    with patch.object(
                        engine, "_handle_complex_workflow"
                    ) as mock_complex:
                        mock_complex.return_value = (
                            f"wf_fallback_{scenario['name'].lower().replace(' ', '_')}"
                        )
                        ...
                        if "execution_error" in scenario:
                            with patch.object(
                                ...
                                if scenario["should_fallback"]:
                                    ...
                                    print(
                                        f"✓ Fallback scenario: {scenario['name']} - {scenario['reason']}"
                                    )
                        else:
                            request = RequestMetadata(
                                prompt="Test fallback scenario",
    ...
    def test_fast_path_disabled_validation(self, mock_llm_factory, mock_message_bus):
        ...
        with patch("supervisor.workflow_engine.MCPClientManager"):
            ...
            with patch.object(engine, "_handle_complex_workflow") as mock_complex:
                ...
                print(
                    "✓ Fast-path disabled validation: All requests routed to complex workflow"
                )
    ...
    def test_role_registry_fast_reply_methods_validation(self):
        ...
        print(
            f"✓ RoleRegistry validation: {len(fast_roles)} fast-reply roles properly configured"
        )
    ...
    def test_request_router_validation(self, mock_llm_factory):
        ...
        print(
            f"✓ RequestRouter validation: {stats['available_fast_reply_roles']} roles available"
        )
...
class TestSystemIntegrationValidation:
    ...
    def test_end_to_end_system_validation(self):
        ...
        config = FastPathRoutingConfig.from_dict(
            {"enabled": True, "confidence_threshold": 0.7, "fallback_on_error": True}
        )

        print(
            f"✓ Configuration: enabled={config.enabled}, threshold={config.confidence_threshold}"
        )
