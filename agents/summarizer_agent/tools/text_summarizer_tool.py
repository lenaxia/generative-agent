...
class TextSummarizerTool(BaseTool, BaseModel):
    ...
    def _run(self, text: str, max_summary_length: int = 500) -> Dict[str, Any]:
        ...
        summary_chain = LLMChain(llm=self.llm.get_client(), prompt=summary_prompt)
        ...
        validation_chain = LLMChain(llm=self.llm.get_client(), prompt=validation_prompt)

        # Assuming validation_chain.run returns a dictionary
        validation_output = validation_chain.run(summary=summary, text=text)
        validation_result = SummaryValidationOutput.parse_obj(validation_output)
